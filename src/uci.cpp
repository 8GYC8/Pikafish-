...
void UCIEngine::loop() {
    ...
    // Fine-tune UCI options from the optional external config file before the
    // UCI handshake. "pikafish.toml" is searched next to the binary first, then
    // in the current working directory. Options sent later by the GUI override
    // these values.
    if (cli.argc > 0)
    {
        const std::vector<std::filesystem::path> configPaths = {
          CommandLine::get_binary_directory(cli.argv[0]) / "pikafish.toml",
          std::filesystem::current_path() / "pikafish.toml"};

        for (const auto& path : configPaths)
            if (std::filesystem::exists(path))
            {
                const auto status = engine.get_options().load_config_file(path);
                if (status)
                    print_info_string(*status);
                break;
            }
    }
...
void UCIEngine::position(std::istringstream& is) {
    ...
    {
        // Default to "position fen": when the "fen" keyword is omitted,
        // treat the remaining tokens as a FEN string.
        fen += token + " ";
        while (is >> token && token != "moves")
            fen += token + " ";
    }
